...
Deno.serve(async (req) => {
  ...
  if (ALLOWED_ORIGINS.length === 0) {
    ...
    return new Response(JSON.stringify({ insights: null, error: "api_error" }), {
      status: 500,
  ...
  if (req.method !== "POST") {
    return jsonResponse({ insights: null, error: "api_error" }, origin)
  }
    ...
    return jsonResponse(
      {
        ...
        insights: {
          eating: {
            ...
            cuisine_breakdown: cuisineBreakdown(entries),
          ...
          places: {
            ...
            geographic_range: geographicRange(entries),
            best_rated_venues: bestRatedVenues(entries),
          },
          home_vs_out: {
            cooking_ratio: cookingRatio(entries),
          ...
          timing: {
            ...
            logging_streak: loggingStreak(entries),
